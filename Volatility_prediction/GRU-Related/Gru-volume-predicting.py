...
if __name__ == "__main__":
    ...
    regressor = build_GRU_model(X_train, y_train,indicators, timesteps, prediction_length,filters=64, kernel_size=3,
                                 units=256, dropout_rate=0.2, epochs=25, batch_size= 32)
    ...
    plot_predictions_sep(data_comparision, prediction_length,
                         save_path=r'C:\Users\26876\Desktop\lstmgarch_project\Original_crydata\GRU_family_result\gru_volume_volatility_predictions.png')
    ...
    # 保存训练好的模型
    model_path = os.path.join(save_dir, 'gru_volume_model.h5')
    regressor.save(model_path)  # 将模型保存到指定路径

    # 将 MAPE 和 MSE 列表保存为 JSON 文件
    mape_path = os.path.join(save_dir, 'gru_volume_mape_list.json')
    with open(mape_path, 'w') as f:
        json.dump(mape_list, f)

    mse_path = os.path.join(save_dir, 'gru_volume_mse_list.json')
